...
pub const Validators = struct {
    ...
    pub fn dirExists(value: Argument.ArgValue) Error.FlashError!void {
        const path = value.asString();
        const io = std.Io.Threaded.global_single_threaded.io();
        const dir = std.Io.Dir.cwd().openDir(io, path, .{}) catch {
            return Error.FlashError.ValidationError;
        };
        dir.close();
    }
...
pub fn CrossFieldValidator(comptime T: type) type {
    return struct {
        validate_fn: *const fn (T) Error.FlashError!void,
        error_message: []const u8,

        pub fn init(validate_fn: *const fn (T) Error.FlashError!void, error_message: []const u8) @This() {
            return .{
...
pub fn validateStruct(comptime T: type, instance: T, validators: []const CrossFieldValidator(T)) !ValidationResult {
...
pub const CrossFieldValidators = struct {
    ...
    pub fn dateRange(comptime T: type, comptime start_field: []const u8, comptime end_field: []const u8) *const fn (T) Error.FlashError!void {
        return struct {
            fn validate(instance: T) Error.FlashError!void {
    ...
    pub fn minMaxRange(comptime T: type, comptime min_field: []const u8, comptime max_field: []const u8) *const fn (T) Error.FlashError!void {
        return struct {
            fn validate(instance: T) Error.FlashError!void {
    ...
    pub fn atLeastOne(comptime T: type, comptime fields: []const []const u8) *const fn (T) Error.FlashError!void {
        return struct {
            fn validate(instance: T) Error.FlashError!void {
    ...
    pub fn exactlyOne(comptime T: type, comptime fields: []const []const u8) *const fn (T) Error.FlashError!void {
        return struct {
            fn validate(instance: T) Error.FlashError!void {
                var count: usize = 0;
                inline for (fields) |field| {
...
test "cross-field validation" {
    ...
    const validators = [_]CrossFieldValidator(TestStruct){
        CrossFieldValidator(TestStruct).init(CrossFieldValidators.minMaxRange(TestStruct, "min_value", "max_value"), "min_value must be less than max_value"),
    };
