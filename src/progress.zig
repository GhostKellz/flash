...
pub const ProgressBar = struct {
    ...
    fn render(self: ProgressBar) void {
        ...
        while (i < self.config.width) : (i += 1) {
            if (i < filled) {
                std.debug.print("{s}", .{self.config.fill_char});
            } else {
                std.debug.print("{s}", .{self.config.empty_char});
            }
        }
        std.debug.print("]", .{});
        ...
        if (self.config.show_rate) {
            const elapsed = getMilliTimestamp() - self.start_time;
...
pub const Progress = struct {
    ...
    pub fn withProgressBar(
        ...
        progress.finish();
        if (success_msg) |msg| {
            if (shouldRender()) std.debug.print("✅ {s}\n", .{msg});
        }
