...
pub const Validators = struct {
    ...
    pub fn stringLength(comptime min: ?usize, comptime max: ?usize) ValidatorFn {
        return struct {
    ...
    pub fn numberRange(comptime min: ?i64, comptime max: ?i64) ValidatorFn {
        return struct {
            fn validate(value: Argument.ArgValue) ValidationResult {
    ...
    pub fn regex(comptime pattern: []const u8) ValidatorFn {
        return struct {
            fn validate(value: Argument.ArgValue) ValidationResult {
                const str = value.asString();
